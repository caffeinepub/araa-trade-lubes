...
export function useAddVendor() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      if (!actor) throw new Error('Actor not available');
      return actor.addVendor(params.name, params.phone, params.email, params.address);
    },
    onSuccess: () => {
...
export function useUpdateVendor() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      if (!actor) throw new Error('Actor not available');
      return actor.updateVendor(params.id, params.name, params.phone, params.email, params.address);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vendors'] });
    },
...
export function useAddInvoice() {
  ...
  return useMutation({
    mutationFn: async (params: { customerId: bigint; items: InvoiceItem[] }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useGetCustomerFinancialSummary(customerId: bigint | null) {
  ...
  return useQuery<FinancialSummary>({
    ...
    queryFn: async () => {
      if (!actor || !customerId) throw new Error('Actor or customerId not available');
